...
check_quiz_dir <- function(path = ".",
  ...
  check_quizzes(quiz_dir = file.path(root_dir, quiz_dir), write_report = TRUE, verbose = TRUE)
...
#'
#' @param quiz_specs The output from [parse_quiz].
...
#'
#' This is ran automatically by [check_all_questions] for all questions.
#' It checks that the attributes specified are accepted ones by Leanpub.
#'
#' @param question_df a data.frame obtained from [parse_quiz_df] and dplyr::group_split(question).
...
#'
#' Takes output from [parse_quiz] and runs checks on each question in a quiz by calling [check_question] for each question.
#' First splits questions into their own data frame. Returns a list of messages/warnings about each question's set up.
#'
#' @param quiz_specs quiz_specs which is output from [parse_quiz].
...
#' Is called by [check_all_questions] and run for each question.
#'
...
#' @param quiz_dir A path to a directory full of quizzes that should all be checked with [check_all_quizzes].
#' @param verbose print diagnostic messages
